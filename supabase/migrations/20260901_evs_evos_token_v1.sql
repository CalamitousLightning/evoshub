-- ============================================================
-- EVS (EVOS) Token V1 — MINING WALLET SCHEMA
...
-- for precedent: new products add their own evs_* tables here
-- rather than standing up a separate database).
--
-- Isolation guarantee: nothing in this file touches, references,
-- or writes to agent_wallets / agent_transactions / orders. EVS
...
CREATE TABLE IF NOT EXISTS evs_config (
...
INSERT INTO evs_config (id) VALUES (1) ON CONFLICT (id) DO NOTHING;

CREATE TABLE IF NOT EXISTS evs_allocations (
    id                      SMALLINT PRIMARY KEY DEFAULT 1 CHECK (id = 1),
    ...
    updated_at              TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_by              BIGINT REFERENCES users(id),
    CONSTRAINT evs_allocations_sum_within_target CHECK (
        mining_allocation + sale_allocation + community_allocation
        + liquidity_allocation + treasury_allocation <= total_target
    ),
    CONSTRAINT evs_mining_distributed_within_allocation CHECK (
        mining_distributed <= mining_allocation
    ),
    CONSTRAINT evs_mining_distributed_non_negative CHECK (mining_distributed >= 0)
);
INSERT INTO evs_allocations (id) VALUES (1) ON CONFLICT (id) DO NOTHING;

CREATE TABLE IF NOT EXISTS evs_mining_config (
    id                      SMALLINT PRIMARY KEY DEFAULT 1 CHECK (id = 1),
    ...
    updated_at              TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_by              BIGINT REFERENCES users(id),
    CONSTRAINT evs_mining_reward_bounds_valid CHECK (min_reward_per_session <= max_reward_per_session)
);
INSERT INTO evs_mining_config (id) VALUES (1) ON CONFLICT (id) DO NOTHING;

-- Present but inert in V1 — every write path is blocked in application
-- code while evs_config.sale_enabled = false (see app/evs/routes.py).
CREATE TABLE IF NOT EXISTS evs_sale_config (
    id                  SMALLINT PRIMARY KEY DEFAULT 1 CHECK (id = 1),
    sale_enabled        BOOLEAN NOT NULL DEFAULT false,
...
INSERT INTO evs_sale_config (id) VALUES (1) ON CONFLICT (id) DO NOTHING;

-- ------------------------------------------------------------
-- 2. WALLET + LEDGER
-- ------------------------------------------------------------

CREATE TABLE IF NOT EXISTS evs_wallets (
...
CREATE TABLE IF NOT EXISTS evs_transactions (
    id              BIGSERIAL PRIMARY KEY,
    wallet_id       BIGINT NOT NULL REFERENCES evs_wallets(id),
    user_id         BIGINT NOT NULL REFERENCES users(id),
    type            TEXT NOT NULL CHECK (type IN (
                        'MINING_REWARD','EVS_PURCHASE','REFERRAL_REWARD','BONUS',
...
CREATE UNIQUE INDEX IF NOT EXISTS evs_tx_mining_reward_unique
    ON evs_transactions (reference_id)
    WHERE type = 'MINING_REWARD';

CREATE INDEX IF NOT EXISTS idx_evs_tx_user   ON evs_transactions(user_id);
CREATE INDEX IF NOT EXISTS idx_evs_tx_wallet ON evs_transactions(wallet_id);
CREATE INDEX IF NOT EXISTS idx_evs_tx_type   ON evs_transactions(type);

-- ------------------------------------------------------------
-- 3. MINING SESSIONS
-- ------------------------------------------------------------

CREATE TABLE IF NOT EXISTS evs_mining_sessions (
    id                  BIGSERIAL PRIMARY KEY,
    user_id             BIGINT NOT NULL REFERENCES users(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS evs_one_active_session_per_user
    ON evs_mining_sessions(user_id)
    WHERE status = 'ACTIVE';

CREATE INDEX IF NOT EXISTS idx_evs_mining_user   ON evs_mining_sessions(user_id);
CREATE INDEX IF NOT EXISTS idx_evs_mining_status ON evs_mining_sessions(status);

-- ------------------------------------------------------------
-- 4. FUTURE-PHASE TABLES — schema exists, all writes blocked in
--    application code by evs_config feature flags. No route in
--    V1 inserts into these.
-- ------------------------------------------------------------

CREATE TABLE IF NOT EXISTS evs_purchases (
    id              BIGSERIAL PRIMARY KEY,
    user_id         BIGINT NOT NULL REFERENCES users(id),
    evs_amount      NUMERIC(20,4) NOT NULL,
...
CREATE TABLE IF NOT EXISTS evs_withdrawals (
    id              BIGSERIAL PRIMARY KEY,
    user_id         BIGINT NOT NULL REFERENCES users(id),
    evs_amount      NUMERIC(20,4) NOT NULL,
...
CREATE TABLE IF NOT EXISTS evs_referrals (
    id                  BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS evs_admin_actions (
    id          BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS evs_security_events (
    id          BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_evs_security_user ON evs_security_events(user_id);
CREATE INDEX IF NOT EXISTS idx_evs_security_type ON evs_security_events(event_type);
...
BEGIN
    ...
    SELECT * INTO v_wallet FROM evs_wallets WHERE user_id = p_user_id FOR UPDATE;
    IF NOT FOUND THEN
        INSERT INTO evs_wallets (user_id) VALUES (p_user_id) RETURNING * INTO v_wallet;
    END IF;
    ...
    BEGIN
        INSERT INTO evs_mining_sessions (user_id, expires_at, rate_snapshot, estimated_reward)
...
BEGIN
    ...
    SELECT * INTO v_alloc FROM evs_allocations WHERE id = 1 FOR UPDATE;
    ...
    IF v_reward <= 0 THEN
        UPDATE evs_mining_sessions SET status = 'FLAGGED' WHERE id = p_session_id;
    ...
    INSERT INTO evs_transactions (wallet_id, user_id, type, amount, direction, reference_id, metadata)
...
BEGIN
    ...
    INSERT INTO evs_transactions (wallet_id, user_id, type, amount, direction, metadata)
    ...
    INSERT INTO evs_admin_actions (admin_id, action, old_value, new_value, reason)
...
FROM evs_wallets w
LEFT JOIN evs_transactions t ON t.wallet_id = w.id AND t.status = 'CONFIRMED'
