const API=window.EVS_API_BASE||((location.hostname==='localhost'||location.hostname==='127.0.0.1')?'http://localhost:8000':'https://api.evoshub.xyz');
const $=id=>document.getElementById(id);
let mining=null;
const token=()=>localStorage.getItem('evs_evos_token')||'';
...
async function login(e){
    ...
    try{
        const d=await req('/api/evs/auth/login',{
        ...
        localStorage.setItem('evs_evos_token',d.token);
        localStorage.setItem('evs_evos_user',JSON.stringify(d.user));
        await load()
    }
    catch(err){
...
async function load(){
    try{
        const [w,m,t]=await Promise.all([req('/api/evs/wallet'),req('/api/evs/mining/status'),req('/api/evs/transactions?limit=20&offset=0')]);
        ...
        $('transactions').innerHTML=list.length?list.map(x=>`<div class="tx"><span><b>${x.type.replaceAll('_',' ')}</b><br><small>${new Date(x.created_at).toLocaleString()}</small></span><span class="${x.direction==='CREDIT'?'credit':''}">${x.direction==='CREDIT'?'+':'-'}${fmt(x.amount)} EVS<br><small>${x.status}</small></span></div>`).join(''):'<p class="muted">No transactions yet.</p>'
    }
    catch(err){
        localStorage.removeItem('evs_evos_token');
...
function renderMining(){
    ...
    if(remain<=0){
        ...
        $('countdown').textContent=`+${fmt(mining.estimated_reward)} EVS READY`;
        $('rewardText').textContent='Your session is complete and ready for server verification.';
        b.textContent='CLAIM EVS'
    }
    else{
        ...
        $('rewardText').textContent=`+${fmt(mining.estimated_reward)} EVS estimated for this session`;
...
$('miningAction').onclick=async()=>{
    ...
    try{
        if(!mining){
            const d=await req('/api/evs/mining/start',{
                method:'POST',body:'{}'
            }
            );
            mining=d.mining
            ...
            await req('/api/evs/mining/claim',{
                method:'POST',body:JSON.stringify({
