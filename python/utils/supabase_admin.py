...
# website_chat_messages.sender_id is uuid NOT NULL, with no FK to auth.users
# visible in the RLS policies — so any uuid literal satisfies the column; it
# doesn't have to be a real auth.users id. Admin agents have no Supabase
# Auth uuid of their own (they sign in against public.users, a bigint
# table), so every admin-sent message uses this one fixed placeholder uuid
# instead. sender_role='admin' is what actually marks a message as
# admin-authored; this uuid just satisfies the NOT NULL constraint.
#
# If you later want to know *which* agent sent a given reply, add a
# separate `admin_user_id bigint` column referencing public.users.id
# directly, rather than trying to encode agent identity into this column.
ADMIN_SYSTEM_SENDER_ID = "00000000-0000-0000-0000-000000000001"


async def insert_admin_chat_message(request_id: str, body: str) -> dict:
    """Inserts an admin-sent chat message using the fixed placeholder
    sender_id described above (column is NOT NULL, no FK enforced)."""
    if not SUPABASE_URL or not SUPABASE_SERVICE_ROLE_KEY:
        raise RuntimeError("SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY not configured.")
    ...
    async with httpx.AsyncClient(timeout=_TIMEOUT) as client:
        resp = await client.post(
            ...
            json={
                ...
                "sender_id": ADMIN_SYSTEM_SENDER_ID,
                "sender_role": "admin",
                "body": body,
            },
        )
