...
dot-separated), but with its own secret (EVS_TOKEN_SECRET) so a leak of
ADMIN_SECRET or evoshub's ADMIN_TOKEN_SECRET can't be used to forge EVS
...
def _secret() -> bytes:
    secret = os.getenv("EVS_TOKEN_SECRET", "")
    if not secret:
        raise RuntimeError("EVS_TOKEN_SECRET not configured.")
...
class EvsTokenInvalid(Exception):
    """Missing, malformed, tampered, or expired EVS session token."""


def verify_user_token(token: str) -> int:
    """Returns the verified user id if the token checks out."""
    if not token or "." not in token:
        raise EvsTokenInvalid("Missing or malformed token.")
    ...
    try:
        ...
        raise EvsTokenInvalid("Malformed token signature.")

    if not hmac.compare_digest(expected_sig, given_sig):
        raise EvsTokenInvalid("Invalid token signature.")

    try:
        payload = json.loads(_b64decode(payload_b64))
    except Exception:
        raise EvsTokenInvalid("Malformed token payload.")

    if payload.get("exp", 0) < time.time():
        raise EvsTokenInvalid("Token expired.")

    user_id = payload.get("uid")
    if not isinstance(user_id, int):
        raise EvsTokenInvalid("Token missing user id.")
