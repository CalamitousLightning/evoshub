"""
EVS admin API — NOT discoverable.
...
require_active_admin() below, not the path name.
    ...
    app.include_router(evs_admin_router, prefix="/phasepage/evs")
...
existing /phasepage/* convention used elsewhere in the ecosystem.
"""
...
from evs.config import get_config, get_mining_config, get_allocations
...
def _log_admin_action(admin_id: int, action: str, old_value: dict, new_value: dict, reason: str | None = None):
    supabase.table("evs_admin_actions").insert({
...
def admin_stats(request: Request, authorization: str = Header(default="")):
    ...
    active = supabase.table("evs_mining_sessions").select("id", count="exact").eq("status", "ACTIVE").execute()
    total_wallets = supabase.table("evs_wallets").select("id", count="exact").execute()
    return {
        "status": "ok",
...
class ConfigUpdateRequest(BaseModel):
    updates: dict = Field(..., description="Subset of evs_config fields to change")
...
def admin_update_config(request: Request, data: ConfigUpdateRequest, authorization: str = Header(default="")):
    ...
    supabase.table("evs_config").update(payload).eq("id", 1).execute()
    new = get_config()
...
def admin_update_allocations(request: Request, data: AllocationUpdateRequest, authorization: str = Header(default="")):
    ...
    old = get_allocations()
    try:
        # The CHECK constraints on evs_allocations (sum <= total_target,
        # mining_distributed <= mining_allocation) do the real enforcement —
        # this call simply surfaces a clean error if they're violated.
        supabase.table("evs_allocations").update({**data.updates, "updated_by": admin_id}).eq("id", 1).execute()
...
def admin_update_mining_config(request: Request, data: MiningConfigUpdateRequest, authorization: str = Header(default="")):
    ...
    supabase.table("evs_mining_config").update({**data.updates, "updated_by": admin_id}).eq("id", 1).execute()
...
def admin_adjust_balance(request: Request, user_id: int, data: AdjustBalanceRequest, authorization: str = Header(default="")):
    admin_id = _require_admin(request, authorization)
    try:
        res = supabase.rpc("evs_admin_adjust_balance", {
...
def admin_suspend_wallet(request: Request, user_id: int, data: SuspendRequest, authorization: str = Header(default="")):
    ...
    old = supabase.table("evs_wallets").select("status").eq("user_id", user_id).limit(1).execute()
    supabase.table("evs_wallets").update({"status": new_status}).eq("user_id", user_id).execute()
...
def admin_security_events(request: Request, authorization: str = Header(default=""), limit: int = 50):
    _require_admin(request, authorization)
    res = supabase.table("evs_security_events").select("*").order("created_at", desc=True).limit(min(limit, 200)).execute()
...
def admin_actions_log(request: Request, authorization: str = Header(default=""), limit: int = 50):
    _require_admin(request, authorization)
    res = supabase.table("evs_admin_actions").select("*").order("created_at", desc=True).limit(min(limit, 200)).execute()
...
def admin_reconciliation(request: Request, authorization: str = Header(default="")):
    _require_admin(request, authorization)
    res = supabase.table("evs_reconciliation_report").select("*").execute()
