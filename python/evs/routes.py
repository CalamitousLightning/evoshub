"""
EVS user-facing API — mounted in main.py as:

    from evs.routes import router as evs_router
    app.include_router(evs_router, prefix="/api/evs", tags=["evs"])

Every route here requires a valid EVS session token (see user_auth.py),
...
from evs.user_auth import verify_user_token, EvsTokenInvalid
from evs.config import get_config, get_allocations
from evs.wallet import get_or_create_wallet, get_transactions
from evs.mining import start_session, get_status, claim, current_rate, MiningError
...
def _log_security_event(user_id: int | None, event_type: str, request: Request, details: dict | None = None):
    try:
        supabase.table("evs_security_events").insert({
            "user_id": user_id,
        ...
        logger.error("EVS SECURITY EVENT LOG FAILED: %s", str(e))
