"""
EVS mining rate + session service.
...
two Postgres RPC functions in migrations/2026_evs_v1.sql, called via
...
from evs.wallet import get_or_create_wallet
...
def _active_miners_count() -> int:
    res = supabase.table("evs_mining_sessions").select("id", count="exact").eq("status", "ACTIVE").execute()
...
def start_session(user_id: int) -> dict:
    ...
    try:
        res = supabase.rpc("evs_start_mining_session", {
            "p_user_id": user_id,
...
def get_status(user_id: int) -> dict | None:
    res = (
        supabase.table("evs_mining_sessions")
        .select("*")
        .eq("user_id", user_id)
        .eq("status", "ACTIVE")
...
def claim(user_id: int, session_id: int) -> dict:
    ...
    try:
        res = supabase.rpc("evs_claim_mining_reward", {
