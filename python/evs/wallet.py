...
through the evs_claim_mining_reward / evs_admin_adjust_balance Postgres
functions in the migration, which are the only things allowed to touch
evs_wallets.cached_balance. This module is read-only on purpose.
"""
...
def get_or_create_wallet(user_id: int) -> dict:
    res = supabase.table("evs_wallets").select("*").eq("user_id", user_id).limit(1).execute()
    if res.data:
        return res.data[0]
    # Insert-if-missing race is fine here: user_id is UNIQUE on evs_wallets,
    # so a concurrent duplicate insert fails and we just re-read.
    try:
        created = supabase.table("evs_wallets").insert({"user_id": user_id}).execute()
...
def get_transactions(user_id: int, limit: int = 50, offset: int = 0) -> list[dict]:
    res = (
        supabase.table("evs_transactions")
